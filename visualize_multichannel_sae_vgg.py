"""
Multi-Channel ConvSAE Visualization Script (VGG16) - Two-Level Sparsity

For a given input image:
1. Extract VGG16 features[16] activations (256 channels, 28×28)
...
7. Visualize these feature maps directly (28×28 heatmaps)

Usage:
    # Visualize single image
    python visualize_multichannel_sae_vgg.py --image_path data/imagenette/tench/n01440764_1.JPEG

    # Visualize multiple images from a class
    python visualize_multichannel_sae_vgg.py --class_name tench --num_images 3

    # Customize parameters
    python visualize_multichannel_sae_vgg.py --class_name gas_pump --num_images 5 --top_k_features 16
"""
...
sys.path.append('.')
from run_csae_vgg import MultiChannelConvSAE
from src.gradcam import GradCAM


class MultiChannelSAEVisualizerVGG:
    """
    Visualizer for Multi-Channel ConvSAE learned features (VGG16 backbone).
    Uses model with two-level sparsity mechanism.

    For each input image:
    1. Extracts VGG16 features[16] activations (256 channels, 28×28)
    ...
    6. Visualizes feature maps directly as 28×28 heatmaps
    """

    def __init__(self,
                 csae_model_path: str = 'multichannel_csae_vgg_model.pkl',
                 device='cuda',
    ...
    def _plot_feature_grid(self, image: Image.Image, top_features: List[Tuple],
        ...
        plt.suptitle(f'Multi-Channel ConvSAE (VGG16): Top-{n_features} Activated Features ({self.spatial_size}×{self.spatial_size} Heatmaps)\n' +
...
def visualize_multichannel_sae_vgg(
    model: MultiChannelConvSAE,
    sample_activations: torch.Tensor,
    num_samples: int = 4,
    num_channels_to_show: int = 8,
    num_features_to_show: int = 16,
    save_path: str = 'multichannel_sae_vgg_visualization.png'
):
    """
    Comprehensive visualization of Multi-Channel ConvSAE (VGG16) learned features.

    Shows:
    1. Sample input activation maps (selected channels)
    2. Top-k CSAE features with highest activation
    3. Decoder weight analysis (channel importance)

    Args:
        model: Trained MultiChannelConvSAE
        sample_activations: [N, num_channels, H, W] - Sample activation maps from VGG16
    ...
    # Get sparse features for samples
    with torch.no_grad():
    ...
    # ===== Row 0: Sample input activations (selected channels) =====
    for i in range(num_samples):
        ax = fig.add_subplot(gs[0, i])

        # Show averaged selected channels for this sample
        sample_acts = samples[i].cpu()  # [num_channels, H, W]

        # Get channels with non-zero activations (selected by GradCAM)
        active_channels = (sample_acts.sum(dim=(1, 2)) > 1e-8).nonzero(as_tuple=True)[0]

        if len(active_channels) > 0:
            # Average active channels
            avg_acts = sample_acts[active_channels].mean(dim=0)  # [H, W]
        ...
        ax.set_title(f'Sample {i+1}\n({len(active_channels)} active channels)', fontsize=10)
        ax.axis('off')
    ...
    plt.suptitle(f'Multi-Channel ConvSAE (VGG16) Feature Visualization\n' +
                f'Input: {model.in_channels} channels → Hidden: {model.hidden_dim} features (top-{model.top_k} active)',
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualize Multi-Channel ConvSAE learned features (VGG16)'
    )
    parser.add_argument('--csae_model', type=str,
                       default='multichannel_csae_vgg_model.pkl',
    ...
    parser.add_argument('--output_dir', type=str, default='multichannel_sae_vgg_visualizations',
    ...
    # Create visualizer
    print("="*80)
    print("Multi-Channel ConvSAE Feature Visualization (VGG16)")
    print("="*80 + "\n")

    visualizer = MultiChannelSAEVisualizerVGG(
    ...
    for i, img_path in enumerate(image_paths):
        ...
        save_path = output_dir / f"{img_name}_features_vgg.png"
