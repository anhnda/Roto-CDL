"""
Multi-Channel ConvSAE Visualization Script (ResNet18 - Masked Loss Variant)

For a given input image:
1. Extract ResNet18 layer3 activations (ALL 256 channels, no masking)
...
VARIANT: This visualizes models trained with run_resnet_mask.py, which:
- Uses ALL channels as input (no pre-masking)
- Computes reconstruction loss only on GradCAM-selected channels

Usage:
    # Visualize single image
    python visualize_multichannel_sae_resnet_mask.py --image_path data/imagenette/tench/n01440764_1.JPEG

    # Visualize multiple images from a class
    python visualize_multichannel_sae_resnet_mask.py --class_name tench --num_images 3

    # Customize parameters
    python visualize_multichannel_sae_resnet_mask.py --class_name gas_pump --num_images 5 --top_k_features 16
"""
...
class MultiChannelSAEVisualizerResNetMask:
    """
    Visualizer for Multi-Channel ConvSAE learned features (ResNet18 masked loss variant).

    VARIANT: For models trained with run_resnet_mask.py:
    ...
    1. Extracts ResNet18 layer3 activations (ALL 256 channels)
    ...
    def __init__(self,
                 csae_model_path: str = 'multichannel_csae_resnet_mask_model.pkl',
                 device='cuda',
    ...
    def extract_features(self, image_path: str, top_k: int = 16) -> Dict:
        ...
        Pipeline:
        1. Extract ALL activation channels from ResNet18
    ...
    def _plot_feature_grid(self, image: Image.Image, top_features: List[Tuple],
        ...
        plt.suptitle(f'Multi-Channel ConvSAE (ResNet18 - Masked Loss): Top-{n_features} Activated Features ({self.spatial_size}×{self.spatial_size})\n' +
...
def visualize_multichannel_sae_resnet_mask(
    model: MultiChannelConvSAE,
    sample_activations: torch.Tensor,
    num_samples: int = 4,
    num_channels_to_show: int = 8,
    num_features_to_show: int = 16,
    save_path: str = 'multichannel_sae_resnet_mask_visualization.png'
):
    """
    Comprehensive visualization of Multi-Channel ConvSAE (ResNet18 - Masked Loss) learned features.
    ...
    plt.suptitle(f'Multi-Channel ConvSAE (ResNet18 - Masked Loss Variant) Feature Visualization\n' +
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualize Multi-Channel ConvSAE learned features (ResNet18 - Masked Loss Variant)'
    )
    ...
    parser.add_argument('--output_dir', type=str, default='multichannel_sae_resnet_mask_visualizations',
    ...
    # Create visualizer
    print("="*80)
    print("Multi-Channel ConvSAE Feature Visualization (ResNet18 - Masked Loss Variant)")
    print("="*80 + "\n")

    visualizer = MultiChannelSAEVisualizerResNetMask(
    ...
    for i, img_path in enumerate(image_paths):
        ...
        save_path = output_dir / f"{img_name}_features_resnet_mask.png"
