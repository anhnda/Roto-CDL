"""
Multi-Channel ConvSAE Training Script with VGG16 Backbone (Two-Level Sparsity)

Uses pretrained VGG16 (ImageNet) to extract activation channels from a middle layer.
...
Key Design:
- Backbone: VGG16 pretrained on ImageNet-1k
- Target layer: features[23] (middle layer after 3rd conv block, 256 channels, 28×28 resolution)
...
This approach treats the VGG channels as a "vocabulary" and learns
...
class MultiChannelConvSAE(nn.Module):
    ...
    Args:
        in_channels: Number of input channels (e.g., 256 for VGG16 features[23])
...
# ==========================================
# VGG16 Activation Extractor
# ==========================================

class VGG16ActivationExtractor:
    """
    Extracts activation channels from VGG16 middle layer with GradCAM-based selection.

    Target Layer: features[23] (after 3rd conv block, middle of the network)
    - VGG16 features[23]: 256 channels, 28×28 spatial resolution

    For each image:
    1. Compute GradCAM scores for all channels at features[23]
    2. Select channels with cumulative score ≥ threshold (default 85%)
    ...
    def __init__(self, device='cuda', cumulative_threshold=0.85):
        ...
        # Target layer: features[23] (middle layer after 3rd conv block)
        # VGG16 structure:
        # - features[0-4]: Conv block 1 (64 channels, maxpool at [4])
        # - features[5-9]: Conv block 2 (128 channels, maxpool at [9])
        # - features[10-16]: Conv block 3 (256 channels, maxpool at [16])
        # - features[17-23]: Conv block 4 (512 channels, maxpool at [23])
        # - features[24-30]: Conv block 5 (512 channels, maxpool at [30])
        # We use features[16] (after 3rd conv block, before maxpool) for 256 channels at 28×28
    ...
    def _get_cache_filename(self, data_loader: DataLoader, normalize: bool) -> str:
        ...
        config_str = f"vgg16_features16_threshold{self.cumulative_threshold}_normalize{normalize}_nsamples{len(data_loader.dataset)}"
        config_hash = hashlib.md5(config_str.encode()).hexdigest()[:8]

        cache_dir = "cache_activations"
        os.makedirs(cache_dir, exist_ok=True)

        cache_filename = os.path.join(cache_dir, f"vgg16_activations_{config_hash}.pt")
    ...
    def collect_activation_maps(
        ...
        - Select channels with cumulative score ≥ threshold (default 85%)
        ...
        print(f"Collecting activation maps from VGG16 features[16] with GradCAM selection...")
...
def plot_training_logs(logs: Dict[str, List], save_path: str = 'multichannel_csae_vgg_logs.png'):
    """Plot training metrics."""
    fig, axs = plt.subplots(3, 3, figsize=(18, 12))
    fig.suptitle('Multi-Channel ConvSAE Training (VGG16 - Two-Level Sparsity)',
...
def visualize_learned_features(model: MultiChannelConvSAE,
                               num_features: int = 32,
                               save_path: str = 'multichannel_csae_vgg_features.png'):
    """Visualize learned decoder features."""
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    fig.suptitle('Multi-Channel ConvSAE Learned Features (VGG16)', fontsize=14, fontweight='bold')
...
if __name__ == "__main__":
    ...
    # ========================================
    print("="*80)
    print("Multi-Channel ConvSAE Training with VGG16 Backbone")
    print("="*80)
    print("\nSetting up Data...")
    ...
    print(f"  • GradCAM: Variable channels selected per image (85% cumulative score)")
    print("="*80 + "\n")
    ...
    torch.save(csae_model.state_dict(), 'multichannel_csae_vgg_model.pth')
    print("✓ Model state dict: multichannel_csae_vgg_model.pth")

    joblib.dump(csae_model.cpu(), 'multichannel_csae_vgg_model.pkl')
    print("✓ Full model: multichannel_csae_vgg_model.pkl")
    ...
    joblib.dump(training_info, 'multichannel_csae_vgg_training_info.pkl')
    print("✓ Training info: multichannel_csae_vgg_training_info.pkl")
    ...
    plot_training_logs(logs, save_path='multichannel_csae_vgg_logs.png')
    csae_model = csae_model.to(device)
    visualize_learned_features(csae_model, num_features=64,
                               save_path='multichannel_csae_vgg_features.png')
    ...
    visualize_multichannel_sae_vgg(
        ...
        save_path='multichannel_sae_vgg_visualization.png'
    )

    print("\n" + "="*80)
    print("✓ All done! Outputs:")
    print("  - multichannel_csae_vgg_model.pth")
    print("  - multichannel_csae_vgg_model.pkl")
    print("  - multichannel_csae_vgg_training_info.pkl")
    print("  - multichannel_csae_vgg_logs.png")
    print("  - multichannel_csae_vgg_features.png")
    print("  - multichannel_sae_vgg_visualization.png")
    print("="*80)
