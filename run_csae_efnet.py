"""
Multi-Channel ConvSAE Training Script with EfficientNet Backbone (Two-Level Sparsity)

Uses pretrained EfficientNet-B0 (ImageNet) to extract activation channels from a middle layer.
...
- Target layer: features[4] (middle layer after ReLU, ~112 channels, 14×14 resolution)
- Architecture: in_channels → 8× sparse features (1×1 conv)
- Two-Level Sparsity:
...
This approach treats the EfficientNet channels as a "vocabulary" and learns
...
from visualize_efnet import visualize_multichannel_sae_efnet
...
class MultiChannelConvSAE(nn.Module):
    ...
    Args:
        in_channels: Number of input channels (e.g., 112 for EfficientNet-B0 features[4])
        hidden_dim: Number of sparse features (e.g., 896 for 8× expansion)
    ...
    def __init__(self, in_channels: int = 112, hidden_dim: int = 896,
...
# ==========================================
# EfficientNet Activation Extractor
# ==========================================

class EfficientNetActivationExtractor:
    """
    Extracts activation channels from EfficientNet-B0 middle layer with GradCAM-based selection.

    Target Layer: features[4] (after MBConv block 4, middle of the network)
    - EfficientNet-B0 features[4]: 112 channels, 14×14 spatial resolution

    For each image:
    1. Compute GradCAM scores for all channels at features[4]
    2. Select channels with cumulative score ≥ threshold (default 80%)
    ...
    def __init__(self, device='cuda', cumulative_threshold=0.85):
        ...
        # Target layer: features[4] (middle layer after ReLU)
        # EfficientNet-B0 structure:
        # - features[0]: stem (32 channels)
        # - features[1]: MBConv1 (16 channels)
        # - features[2]: MBConv6 (24 channels)
        # - features[3]: MBConv6 (40 channels)
        # - features[4]: MBConv6 (80 channels, 14×14) <- TARGET (middle layer)
        # - features[5]: MBConv6 (112 channels)
        # - features[6]: MBConv6 (192 channels)
        # - features[7]: MBConv6 (320 channels)
        self.target_layer = self.model.features[4]

        # Get the actual number of channels from the target layer
        # EfficientNet blocks have complex internal structure, need to inspect
        # The output channels can be obtained from the last conv in the block
        with torch.no_grad():
            dummy_input = torch.randn(1, 3, 224, 224).to(device)
            # Forward through features up to target layer
            x = dummy_input
    ...
    def _get_cache_filename(self, data_loader: DataLoader, normalize: bool) -> str:
        ...
        config_str = f"efnet_features4_threshold{self.cumulative_threshold}_normalize{normalize}_nsamples{len(data_loader.dataset)}"
        config_hash = hashlib.md5(config_str.encode()).hexdigest()[:8]

        cache_dir = "cache_activations"
        os.makedirs(cache_dir, exist_ok=True)

        cache_filename = os.path.join(cache_dir, f"efnet_activations_{config_hash}.pt")
    ...
    def collect_activation_maps(
        ...
        For each image:
        - Compute GradCAM scores for all channels
        - Select channels with cumulative score ≥ threshold (default 80%)
        - Zero-out unselected channels but keep tensor shape [num_channels, H, W]
        ...
        Returns:
            X: [N, num_channels, H, W] - Activation maps with selective zero-masking
        """
        ...
        print(f"Collecting activation maps from EfficientNet-B0 features[4] with GradCAM selection...")
        ...
        for images, labels in tqdm(data_loader, desc="Extracting activations"):
            ...
            for i in range(images.size(0)):
                ...
                # channel_mask: [num_channels] -> reshape to [1, num_channels, 1, 1] for broadcasting
                mask_4d = channel_mask.view(1, self.num_channels, 1, 1).float()
                masked_activations = activations * mask_4d  # [1, num_channels, H, W]
        ...
        print(f"  Sparsity: {(self.num_channels - avg_selected) / self.num_channels * 100:.1f}% channels zeroed out")
...
def plot_training_logs(logs: Dict[str, List], save_path: str = 'multichannel_csae_efnet_logs.png'):
    """Plot training metrics."""
    fig, axs = plt.subplots(3, 3, figsize=(18, 12))
    fig.suptitle('Multi-Channel ConvSAE Training (EfficientNet-B0 - Two-Level Sparsity)',
...
def visualize_learned_features(model: MultiChannelConvSAE,
                               num_features: int = 32,
                               save_path: str = 'multichannel_csae_efnet_features.png'):
    """Visualize learned decoder features."""
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    fig.suptitle('Multi-Channel ConvSAE Learned Features (EfficientNet-B0)', fontsize=14, fontweight='bold')
...
if __name__ == "__main__":
    ...
    # ========================================
    print("="*80)
    print("Multi-Channel ConvSAE Training with EfficientNet-B0 Backbone")
    print("="*80)
    print("\nSetting up Data...")
    ...
    # Collect activation maps (with caching for faster repeated runs)
    ...
    print(f"  Shape: {X.shape}")  # [N, num_channels, H, W]
    ...
    # Hyperparameters
    BATCH_SIZE = 64
    INPUT_CHANNELS = extractor.num_channels  # Channels from EfficientNet features[4]
    ...
    print(f"  • Channel Sparsity: Each feature uses subset of input channels")
    print(f"  • Feature Maps: Sparse at both levels (few channels × sparse spatial patterns)")
    print(f"  • GradCAM: Variable channels selected per image (80% cumulative score)")
    print("="*80 + "\n")
    ...
    torch.save(csae_model.state_dict(), 'multichannel_csae_efnet_model.pth')
    print("✓ Model state dict: multichannel_csae_efnet_model.pth")

    joblib.dump(csae_model.cpu(), 'multichannel_csae_efnet_model.pkl')
    print("✓ Full model: multichannel_csae_efnet_model.pkl")
    ...
    joblib.dump(training_info, 'multichannel_csae_efnet_training_info.pkl')
    print("✓ Training info: multichannel_csae_efnet_training_info.pkl")
    ...
    plot_training_logs(logs, save_path='multichannel_csae_efnet_logs.png')
    csae_model = csae_model.to(device)
    visualize_learned_features(csae_model, num_features=64,
                               save_path='multichannel_csae_efnet_features.png')

    # Generate comprehensive EfficientNet SAE visualization
    print("\nGenerating EfficientNet Multi-Channel SAE visualization...")
    visualize_multichannel_sae_efnet(
        model=csae_model,
        sample_activations=X,
        num_samples=4,
        num_channels_to_show=8,
        num_features_to_show=16,
        save_path='multichannel_sae_efnet_visualization.png'
    )

    print("\n" + "="*80)
    print("✓ All done! Outputs:")
    print("  - multichannel_csae_efnet_model.pth")
    print("  - multichannel_csae_efnet_model.pkl")
    print("  - multichannel_csae_efnet_training_info.pkl")
    print("  - multichannel_csae_efnet_logs.png")
    print("  - multichannel_csae_efnet_features.png")
    print("  - multichannel_sae_efnet_visualization.png")
    print("="*80)
