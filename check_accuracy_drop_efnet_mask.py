"""
Accuracy Drop Analysis for EfficientNet-B0 Masked Loss CSAE

This script evaluates the impact of replacing original EfficientNet-B0 features[4] activations
...
3. For each test image:
   a. Extract original activations at features[4]
   b. Pass through CSAE to get reconstruction
   c. Replace original with reconstructed activations
   d. Continue forward pass through rest of EfficientNet-B0
...
Usage:
    python check_accuracy_drop_efnet_mask.py
    python check_accuracy_drop_efnet_mask.py --csae_model multichannel_csae_efnet_mask_model.pkl
    python check_accuracy_drop_efnet_mask.py --num_samples 1000
"""
...
class EfficientNetB0WithCSAEReconstruction:
    """
    EfficientNet-B0 model with CSAE reconstruction at features[4].

    Allows evaluation of classification accuracy when original activations
    are replaced with CSAE reconstructions.
    """

    def __init__(self, efnet_model: nn.Module, csae_model: MultiChannelConvSAE, device='cuda'):
        """
        Args:
            efnet_model: Pretrained EfficientNet-B0 model
            csae_model: Trained Multi-Channel ConvSAE (masked loss variant)
            device: Device to run on
        """
        self.efnet = efnet_model.to(device)
        ...
        # Hook to capture activations at features[4]
    ...
    def forward_with_reconstruction(self, x: torch.Tensor) -> Tuple[torch.Tensor, Dict]:
        """
        Forward pass with CSAE-reconstructed activations at features[4].
        ...
        """
        with torch.no_grad():
            # Forward through EfficientNet-B0 up to features[4]
            ...
            # Continue forward pass through rest of EfficientNet-B0
            for i in range(5, len(self.efnet.features)):
                x = self.efnet.features[i](x)

            # Classifier
            x = self.efnet.avgpool(x)
            x = torch.flatten(x, 1)
            logits = self.efnet.classifier(x)
...
def evaluate_accuracy_drop(
    model: EfficientNetB0WithCSAEReconstruction,
    ...
    Args:
        model: EfficientNetB0WithCSAEReconstruction model
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate accuracy drop with CSAE-reconstructed activations (EfficientNet-B0 Masked Loss)'
    )
    ...
    parser.add_argument('--output_file', type=str, default='accuracy_drop_efnet_mask_results.txt',
                       help='Output file to save results')

    args = parser.parse_args()

    print("="*80)
    print("EfficientNet-B0 Masked Loss CSAE - Accuracy Drop Analysis")
    print("="*80)
    ...
    print("\nCreating EfficientNetB0WithCSAEReconstruction...")
    model = EfficientNetB0WithCSAEReconstruction(efnet_model, csae_model, device=device)
    print("  ✓ Model ready")
    ...
    with open(args.output_file, 'w') as f:
        f.write("="*80 + "\n")
        f.write("EfficientNet-B0 Masked Loss CSAE - Accuracy Drop Analysis\n")
        f.write("="*80 + "\n\n")
