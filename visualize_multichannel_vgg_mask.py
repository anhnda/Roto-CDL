"""
Multi-Channel ConvSAE Visualization Script (VGG16 - Masked Loss Variant)

For a given input image:
1. Extract VGG16 features[16] activations (ALL 256 channels, no masking)
...
6. Visualize feature maps as 28×28 heatmaps
7. Show which input channels were selected by GradCAM (for reference)

VARIANT: This visualizes models trained with run_vgg_mask.py, which:
- Uses ALL channels as input (no pre-masking)
- Computes reconstruction loss only on GradCAM-selected channels

Usage:
    # Visualize single image
    python visualize_multichannel_vgg_mask.py --image_path data/imagenette/tench/n01440764_1.JPEG

    # Visualize multiple images from a class
    python visualize_multichannel_vgg_mask.py --class_name tench --num_images 3

    # Customize parameters
    python visualize_multichannel_vgg_mask.py --class_name gas_pump --num_images 5 --top_k_features 16
"""
...
class MultiChannelSAEVisualizerVGGMask:
    """
    Visualizer for Multi-Channel ConvSAE learned features (VGG16 masked loss variant).

    VARIANT: For models trained with run_vgg_mask.py:
    ...
    1. Extracts VGG16 features[16] activations (ALL 256 channels)
    ...
    5. Visualizes feature maps as 28×28 heatmaps
    """

    def __init__(self,
                 csae_model_path: str = 'multichannel_csae_vgg_mask_model.pkl',
                 device='cuda',
        ...
        self.target_layer = self.vgg.features[16]
        ...
        print(f"  ✓ Target layer: features[16], {self.num_channels} channels, {self.spatial_size}×{self.spatial_size}")

        # Hook for features[16] activations
        ...
        self.gradcam = GradCAM(self.vgg, self.target_layer)
    ...
    def extract_features(self, image_path: str, top_k: int = 16) -> Dict:
        ...
        Pipeline:
        1. Extract ALL activation channels from VGG16
        ...
        4. Pass ALL channels through CSAE to get sparse features
           - CSAE encoder applies two-level sparsity on full input
        5. Select top-k most activated feature maps
        ...
        Returns:
            results: Dictionary containing:
                ...
                - num_selected_channels: Number of channels selected by GradCAM (reference)
                - channel_mask: Binary mask [num_channels] for GradCAM-selected channels (reference)
        ...
        # Pass ALL channels through CSAE encoder (with Two-Level Sparsity)
        # VARIANT: Model was trained on full input with masked loss
        with torch.no_grad():
    ...
    def _plot_feature_grid(self, image: Image.Image, top_features: List[Tuple],
        ...
        # Column 2-3: Info (GradCAM reference + Variant note)
        ...
        for idx, score in zip(top_5_indices[:3], top_5_scores[:3]):  # Show only top 3 for space
        ...
        plt.suptitle(f'Multi-Channel ConvSAE (VGG16 - Masked Loss): Top-{n_features} Activated Features ({self.spatial_size}×{self.spatial_size})\n' +
...
def visualize_multichannel_sae_vgg_mask(
    model: MultiChannelConvSAE,
    sample_activations: torch.Tensor,
    num_samples: int = 4,
    num_channels_to_show: int = 8,
    num_features_to_show: int = 16,
    save_path: str = 'multichannel_sae_vgg_mask_visualization.png'
):
    """
    Comprehensive visualization of Multi-Channel ConvSAE (VGG16 - Masked Loss) learned features.

    VARIANT: For models trained with run_vgg_mask.py (masked loss variant).

    Shows:
    1. Sample input activation maps (ALL channels, no masking)
    2. Top-k CSAE features with highest activation
    3. Decoder weight analysis (channel importance)

    Args:
        model: Trained MultiChannelConvSAE (masked loss variant)
        sample_activations: [N, num_channels, H, W] - Sample activation maps from VGG16 (full, no masking)
    ...
    samples = sample_activations[indices].to(device)  # [num_samples, num_channels, H, W]

    # Get sparse features for samples (ALL channels processed)
    with torch.no_grad():
        _, sparse_features = model(samples, use_topk=True)  # [num_samples, hidden_dim, H, W]

    # Compute feature importance (sum across spatial and sample dimensions)
    feature_importance = sparse_features.sum(dim=(0, 2, 3)).cpu()  # [hidden_dim]
    top_features_idx = torch.argsort(feature_importance, descending=True)[:num_features_to_show]

    # Get decoder weights
    decoder_weights = model.decoder.weight.detach().cpu().squeeze()  # [in_channels, hidden_dim]

    # Create figure
    fig = plt.figure(figsize=(24, 14))
    gs = fig.add_gridspec(4, num_samples + 2, hspace=0.4, wspace=0.3)

    # ===== Row 0: Sample input activations (ALL channels, no masking) =====
    for i in range(num_samples):
        ax = fig.add_subplot(gs[0, i])

        # Show averaged ALL channels for this sample
        sample_acts = samples[i].cpu()  # [num_channels, H, W]

        # Average ALL channels
        avg_acts = sample_acts.mean(dim=0)  # [H, W]
    ...
    # Row 0, last 2 columns: Channel importance from decoder weights
    ax_channel = fig.add_subplot(gs[0, num_samples:])
    channel_importance = decoder_weights.abs().sum(dim=1).numpy()  # [in_channels]
    top_channels = np.argsort(channel_importance)[-20:][::-1]  # Top 20 channels
    ...
    for feat_idx, global_feat_idx in enumerate(top_features_idx):
        ...
        if row >= 4:  # Limit to 3 rows for features
            break

        ax = fig.add_subplot(gs[row, col])

        # Get average activation map for this feature across samples
        feat_maps = sparse_features[:, global_feat_idx, :, :].cpu()  # [num_samples, H, W]
        avg_feat_map = feat_maps.mean(dim=0)  # [H, W]
    ...
    plt.suptitle(f'Multi-Channel ConvSAE (VGG16 - Masked Loss Variant) Feature Visualization\n' +
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualize Multi-Channel ConvSAE learned features (VGG16 - Masked Loss Variant)'
    )
    parser.add_argument('--csae_model', type=str,
                       default='multichannel_csae_vgg_mask_model.pkl',
    ...
    parser.add_argument('--output_dir', type=str, default='multichannel_sae_vgg_mask_visualizations',
    ...
    # Create visualizer
    print("="*80)
    print("Multi-Channel ConvSAE Feature Visualization (VGG16 - Masked Loss Variant)")
    print("="*80 + "\n")

    visualizer = MultiChannelSAEVisualizerVGGMask(
    ...
    for i, img_path in enumerate(image_paths):
        ...
        save_path = output_dir / f"{img_name}_features_vgg_mask.png"
