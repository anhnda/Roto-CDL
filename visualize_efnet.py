"""
Visualization utilities for EfficientNet-B0 Multi-Channel ConvSAE (Two-Level Sparsity).
...
Sparse Autoencoder trained on EfficientNet-B0 features[4] activations.
...
def visualize_multichannel_sae_efnet(
    model,
    sample_activations: torch.Tensor,
    num_samples: int = 4,
    num_channels_to_show: int = 8,
    num_features_to_show: int = 16,
    save_path: str = 'multichannel_sae_efnet_visualization.png'
):
    """
    Comprehensive visualization of EfficientNet-B0 Multi-Channel SAE with two-level sparsity.
    ...
    Shows:
    1. Sample input activation maps (selected channels from EfficientNet-B0 features[4])
    ...
    Args:
        ...
        sample_activations: [N, num_channels, H, W] - Sample activation maps from EfficientNet-B0 features[4]
    ...
    num_channels = sample_activations.shape[1]
    spatial_size = sample_activations.shape[2]
    ...
    fig.suptitle(f'EfficientNet-B0 Multi-Channel SAE Visualization (Two-Level Sparsity)\n'
                 f'{num_channels} channels, {spatial_size}×{spatial_size} resolution',
    ...
    for i in range(num_samples):
        ...
        input_act = samples[i]  # [num_channels, H, W]
        recon_act = reconstructions[i]  # [num_channels, H, W]
        sparse_feat = sparse_features[i]  # [hidden_dim, H, W]

        # Compute reconstruction error per channel
        recon_error = (input_act - recon_act).pow(2).mean(dim=(1, 2))  # [num_channels]

        # Find most active channels (non-zero input channels)
        channel_activity = input_act.abs().sum(dim=(1, 2))  # [num_channels]
        ...
        summary_text += f"  Active: {len(active_channels)}/{num_channels}\n"
    ...
    print(f"EfficientNet-B0 Multi-Channel SAE visualization saved to {save_path}")
    plt.close()
...
def visualize_feature_decoder_weights(
    model,
    num_features: int = 64,
    save_path: str = 'efnet_decoder_weights.png'
):
    """
    Visualize the decoder weight matrix to understand which EfficientNet-B0 channels
    ...
    top_features = decoder_weights[:, top_feature_indices]  # [in_channels, num_features]

    # Create visualization
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    fig.suptitle(f'EfficientNet-B0 Decoder Feature Weights Analysis (Two-Level Sparsity)\n'
    ...
    ax.set_title(f'Top {min(num_features, len(top_feature_indices))} Feature Weights (Channels × Features)')
    ax.set_xlabel('Feature Index')
    ax.set_ylabel(f'EfficientNet-B0 Channel (0-{in_channels-1})')
    ...
    ax.set_xlabel('EfficientNet-B0 Channel Index')
...
def compare_input_vs_reconstruction(
    ...
    sample_activations: torch.Tensor,
    num_samples: int = 3,
    save_path: str = 'efnet_reconstruction_comparison.png'
):
    """
    Side-by-side comparison of input activations vs reconstructions
    across all channels (averaged).

    Args:
        model: Trained MultiChannelConvSAE model
        sample_activations: [N, num_channels, H, W] - Sample activation maps
    ...
    fig.suptitle(f'EfficientNet-B0 SAE: Input vs Reconstruction Comparison (Two-Level Sparsity)\n'
    ...
    for i in range(num_samples):
        ...
        input_avg = input_act.mean(dim=0)  # [H, W]
        recon_avg = recon_act.mean(dim=0)  # [H, W]
        ...
        axes[i, 0].set_title(f'Sample {i+1}: Input\n({active_channels}/{num_channels} active ch)')
        axes[i, 0].axis('off')
        ...
        sparse_activity = (sparse_features[i] > 0).float().sum(dim=0)  # [H, W]
