"""
Multi-Channel ConvSAE Training Script with ResNet18 Backbone (Masked Loss Variant)
...
- Target layer: layer3 (256 channels, 14×14 resolution)
...
class ResNet18ActivationExtractor:
    ...
    def _get_cache_filename(self, data_loader: DataLoader, normalize: bool) -> str:
        ...
        config_str = f"resnet18_layer3_nomask_threshold{self.cumulative_threshold}_normalize{normalize}_nsamples{len(data_loader.dataset)}"
        config_hash = hashlib.md5(config_str.encode()).hexdigest()[:8]

        cache_dir = "cache_activations"
        os.makedirs(cache_dir, exist_ok=True)

        cache_filename = os.path.join(cache_dir, f"resnet18_activations_nomask_{config_hash}.pt")
    ...
    def collect_activation_maps(
        ...
        print(f"Collecting activation maps from ResNet18 layer3 WITHOUT masking...")
...
def plot_training_logs(logs: Dict[str, List], save_path: str = 'multichannel_csae_resnet_mask_logs.png'):
    """Plot training metrics."""
    fig, axs = plt.subplots(3, 3, figsize=(18, 12))
    fig.suptitle('Multi-Channel ConvSAE Training (ResNet18 - Masked Loss Variant)',
...
def visualize_learned_features(model: MultiChannelConvSAE,
                               num_features: int = 32,
                               save_path: str = 'multichannel_csae_resnet_mask_features.png'):
    """Visualize learned decoder features."""
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    fig.suptitle('Multi-Channel ConvSAE Learned Features (ResNet18 - Masked Loss)', fontsize=14, fontweight='bold')
...
if __name__ == "__main__":
    ...
    # ========================================
    print("="*80)
    print("Multi-Channel ConvSAE Training with ResNet18 Backbone (Masked Loss Variant)")
    print("="*80)
    ...
    print(f"\nExtracting {extractor.num_channels} channels from ResNet18 layer3...")
    ...
    print(f"  Backbone: ResNet18 (pretrained ImageNet-1k)")
    print(f"  Target layer: layer3 ({INPUT_CHANNELS} channels, {extractor.spatial_size}×{extractor.spatial_size} resolution)")
    ...
    torch.save(csae_model.state_dict(), 'multichannel_csae_resnet_mask_model.pth')
    print("✓ Model state dict: multichannel_csae_resnet_mask_model.pth")

    joblib.dump(csae_model.cpu(), 'multichannel_csae_resnet_mask_model.pkl')
    print("✓ Full model: multichannel_csae_resnet_mask_model.pkl")
    ...
    joblib.dump(training_info, 'multichannel_csae_resnet_mask_training_info.pkl')
    print("✓ Training info: multichannel_csae_resnet_mask_training_info.pkl")
    ...
    plot_training_logs(logs, save_path='multichannel_csae_resnet_mask_logs.png')
    csae_model = csae_model.to(device)
    visualize_learned_features(csae_model, num_features=64,
                               save_path='multichannel_csae_resnet_mask_features.png')

    print("\n" + "="*80)
    print("✓ All done! Outputs:")
    print("  - multichannel_csae_resnet_mask_model.pth")
    print("  - multichannel_csae_resnet_mask_model.pkl")
    print("  - multichannel_csae_resnet_mask_training_info.pkl")
    print("  - multichannel_csae_resnet_mask_logs.png")
    print("  - multichannel_csae_resnet_mask_features.png")
    print("="*80)
