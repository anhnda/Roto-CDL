"""
Accuracy Drop Analysis for VGG16 Masked Loss CSAE

This script evaluates the impact of replacing original VGG16 features[16] activations
...
3. For each test image:
   a. Extract original activations at features[16]
   b. Pass through CSAE to get reconstruction
   c. Replace original with reconstructed activations
   d. Continue forward pass through rest of VGG16
...
Usage:
    ...
    python check_accuracy_drop_vgg_mask.py --csae_model multichannel_csae_vgg_mask_model.pkl
    python check_accuracy_drop_vgg_mask.py --num_samples 1000
"""
...
class VGG16WithCSAEReconstruction:
    ...
    def __init__(self, vgg_model: nn.Module, csae_model: MultiChannelConvSAE, device='cuda'):
        """
        Args:
            vgg_model: Pretrained VGG16 model
        ...
        # Hook to capture activations at features[16]
    ...
    def forward_with_reconstruction(self, x: torch.Tensor) -> Tuple[torch.Tensor, Dict]:
        """
        Forward pass with CSAE-reconstructed activations at features[16].
        ...
        """
        with torch.no_grad():
            # Forward through VGG up to features[16]
            features = x
            for i in range(17):  # features[0] through features[16]
                features = self.vgg.features[i](features)

            # Capture original activations
            original_acts = features.clone()  # [B, C, H, W]
            ...
            features = reconstructed_acts

            # Continue forward pass through rest of VGG
            for i in range(17, len(self.vgg.features)):
                features = self.vgg.features[i](features)

            # Classifier
            features = self.vgg.avgpool(features)
            features = torch.flatten(features, 1)
            logits = self.vgg.classifier(features)
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate accuracy drop with CSAE-reconstructed activations (VGG16 Masked Loss)'
    )
    ...
    parser.add_argument('--output_file', type=str, default='accuracy_drop_vgg_mask_results.txt',
                       help='Output file to save results')

    args = parser.parse_args()

    print("="*80)
    print("VGG16 Masked Loss CSAE - Accuracy Drop Analysis")
    print("="*80)
    ...
    print("\nCreating VGG16WithCSAEReconstruction...")
    model = VGG16WithCSAEReconstruction(vgg_model, csae_model, device=device)
    print("  ✓ Model ready")
    ...
    with open(args.output_file, 'w') as f:
        f.write("="*80 + "\n")
        f.write("VGG16 Masked Loss CSAE - Accuracy Drop Analysis\n")
        f.write("="*80 + "\n\n")
